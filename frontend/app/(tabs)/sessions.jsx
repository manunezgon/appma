...
const fetchEnrollments = async () => {
  try {
    ...
    const todayClasses = upcoming
      ...
      .map(e => ({ ...e, isToday: true }))
      .sort((a, b) => a.classDateTime - b.classDateTime);

      const nextClasses = upcoming
        .filter(e => {
          return !todayClasses.some(tc => tc.enrollmentId === e.enrollmentId);
        })
        .map(e => ({ ...e, isToday: false }))
        .sort((a, b) => a.classDateTime - b.classDateTime);
  ...
  const handleUnenroll = async () => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/enrollments/${selectedEnrollment}`, {
        method: "DELETE",
        headers: { Authorization: `Bearer ${user?.token}` },
      });
      if (response.ok) {
        setSections(prev =>
          prev
            .map(section => ({
              ...section,
              data: section.data.filter(e => e.enrollmentId !== selectedEnrollment)
            }))
            .filter(section => section.data.length)
        );
      } else {
        console.error("Error unenrolling:", response.status);
      }
    } catch (error) {
      console.error(error);
    } finally {
      ...
      <Modal visible={confirmVisible} transparent animationType="fade">
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            <Text style={styles.modalText}>¿Seguro que quieres desapuntarte de esta clase?</Text>
            <View style={styles.modalButtons}>
              <TouchableOpacity onPress={() => { setConfirmVisible(false); setSelectedEnrollment(null); }} style={styles.modalButton}>
                <Ionicons name="close" size={28} color="#7c23b0ff" />
              </TouchableOpacity>
              <TouchableOpacity onPress={handleUnenroll} style={styles.modalButton}>
                <Ionicons name="log-out-outline" size={28} color="#FF3B30" />
              </TouchableOpacity>
            </View>
          </View>
